# Zephyr arm Toolchain SDK for Workshop

This SDK provides the arm-zephyr-eabi cross-compiler toolchain for building
Zephyr firmware. It requires the `zephyr` base SDK. Version updates are managed
by Renovate and tracked from upstream
...
toolchain bundle and the arm-zephyr-eabi cross-compiler connected.
...
2. Connect the `arm` plug on `zephyr-sdk-ng` to the `toolchain` slot on this SDK.
...
- Interface: `mount`
- Workshop source: `$SDK`
- Purpose: Provides the arm-zephyr-eabi cross-compiler to the `zephyr` base
  SDK.
